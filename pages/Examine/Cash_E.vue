<template>
  <div>
    <!--表格-->
    <el-table :data="shouyi" style="width: 100%">
      ...
      <el-table-column align="center" prop="userId" label="员工ID" />
      <el-table-column align="center" prop="transferAccounts" label="凭证">
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.transferAccounts == null || scope.row.status == '0'"
            type="text"
            size="small"
            @click="uplodeHandler(scope.row)"
            >上传凭证</el-button
          >
          <el-button
            v-else
            type="text"
            size="small"
            @click="lookdetails(scope.row.transferAccounts)"
            >查看凭证</el-button
          >
        </template>
      </el-table-column>
      <el-table-column align="center" fixed="right" label="操作" width="150">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            size="small"
            @click="examineHandler(scope.row)"
            >审核</el-button
          >
        </template>
      </el-table-column>
    ...
    <!--查看凭证详情-->
    <el-dialog
      style="width: 1000px; margin-left: 250px"
      title="凭证"
      :visible.sync="dialogFormVisible1"
    >
      <div class="line"></div>
      <el-image
        style="width: 300px; height: 300px; margin-left: 80px; margin-top: 28px"
        :src="'http://134.175.100.63:8686/group1/' + this.transferAccounts"
      ></el-image>
    </el-dialog>
    <!--查看凭证详情-->
  </div>
