    ...
    <div class="tab">
      <el-tabs v-model="activeName">
            ...
            <el-col :span="12">
              <div class="demo-basic--circle">
                <div class="block">
                  <el-avatar
                    :size="120"
                    :src="'http://134.175.100.63:8686/' + customers_d.imgPhoto"
                  />
                </div>
              </div>
              <div class="sub-title">{{ this.customers_d.realname }}</div>
              <div class="title">海纳百川，有容乃大</div>
              <div class="telephone">
                联系方式：{{ this.customers_d.telephone }}
              </div>
              <div class="status">状态：{{ this.customers_d.status }}</div>
              <div class="addrrs">
                地址：{{ this.customers_d.province
                }}{{ this.customers_d.city }}
              </div>
              <el-divider />
            </el-col>
          </el-row>
        ...
        <el-tab-pane label="订单信息" name="second">
          <!-- 表格 -->
          <el-table :data="orders_c" stripe style="width: 100%">
            <el-table-column
              align="center"
              prop="id"
              label="订单编号"
            ...
            >
              <template slot-scope="scope">
                <el-button
                  type="text"
                  size="small"
                  @click="detailsHandler(scope.row.id)"
                  >详情</el-button
                >
              </template>
            </el-table-column>
        ...
        <el-tab-pane label="服务地址" name="third">
          <!--表格-->
          <el-table :data="addresses" stripe style="width: 100%">
            ...
            <el-table-column align="center" prop="area" label="区/县" />
            <el-table-column align="center" prop="address" label="街道" />
...
export default {
  ...
  created() {
    ...
    this.findOrderByCustomerId(this.$route.params.id);
  },
  computed: {
    ...mapState("address", ["addresses"]),
    ...mapState("customer", ["customers_d"]),
